$(document).ready(function () {
    ...
    index.prototype = {
        bindEvent: function () {
            ...
            $(document).on("pagecreate", "#snackPage", function () { // 当进入页面二时
                self.page = 1;
                //$(window).scroll(function () {
                //    self.loadmoreSnack($(this));
                //});
                ...
                //待定如果有多个加载时候会不会卡死
              /*  var setdefult2 = setInterval(function () {
                    if (self.soverSnack === 1)
                        clearInterval(setdefult2);
                    else if ($("#snackGoods_List").height() < $(window).height())
                        self.loadmoreSnack($(window));
                    else
                        clearInterval(setdefult2);
                }, 500);*/
            });
       ...
       , loadmore: function (obj) {
        ...
        , loadmoreSnack: function (obj) {
            var self = this;
            if (self.finishedSnack === 0 && self.soverSnack === 0) {
                var scrollTop = $(obj).scrollTop();
                var scrollHeight = $(document).height();
                var windowHeight = $(obj).height();
                if ($(".loadmore").length === 0) {
                    var txt = '<div class="loadmore"><span class="loading"></span>加载中..</div>';
                    $("#snackGoods_List").append(txt);
                }
                /*if (scrollTop + windowHeight - scrollHeight <= 50)*/ {
                    //此处是滚动条到底部时候触发的事件，在这里写要加载的数据，或者是拉动滚动条的操作
                    //防止未加载完再次执行
                    self.finishedSnack = 1;
                    var json = self.getQueryParam();
                    $.ajax({
                        url: '../../../Api/GoodsOper/GoodsSearch',
                        type: "POST",
                            ...
                            if (obj.Exception.Success) {
                                ...
                                for (var i = 0; i < obj.GoodsList.length; i++) {
                                    if (i % 2 === 0) {
                                        ...
                                        r = r.replace("!!!", obj.GoodsList[i].Code);
                                        r = r.replace("!@", obj.GoodsList[i].SaleCount);
                                        $("#snackGoods_List").append(r).trigger("create");
                                    } else {
                                        var l = rightGoodsHtml.replace("@@@", obj.GoodsList[i].Code);
                                        l = l.replace("###", obj.GoodsList[i].GoodsName);
                                        l = l.replace("$$$", obj.GoodsList[i].Price);
                                        l = l.replace("!!!", obj.GoodsList[i].Code);
                                        l = l.replace("!@", obj.GoodsList[i].SaleCount);
                                        $("#snackGoods_List").append(l).trigger("create");
                                    }
                                }
                            } else {
                                alert("eror");
